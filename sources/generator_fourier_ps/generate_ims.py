...
def make_im():
    ...
    im_ps = xp.zeros(im.shape)
    ns = int(xp.random.randint(0, config.source.max_ns))
    coords = xp.random.randint(0, config.n_pix, (2, ns))

    rows, cols = coords[0], coords[1]
    im_ps[rows, cols] = xp.random.random(ns)*xp.sum(im)
    im_f = xp.random.random()*im_ps+xp.random.random()*im

    mea = xp.mean(im_f)  # Calculate standard deviation
    return im_f / mea if mea > 0 else im_f  # Normalize by std if std > 0
